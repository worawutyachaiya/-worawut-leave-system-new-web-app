...
interface TimeRecordConfirmModalProps {
  open: boolean
  onClose: () => void
  onConfirm: () => void
  isLoading?: boolean
}
const TimeRecordConfirmModal = ({ open, onClose, onConfirm, isLoading = false }: TimeRecordConfirmModalProps) => {
      ...
      <DialogContent sx={{ py: 4, px: 5 }}>
        <Box sx={{ textAlign: 'center', alignItems: 'center', display: 'flex', flexDirection: 'column' }}>
          ...
          <Typography variant='h5' sx={{ color: 'text.secondary', mb: 4 }}>
            ยืนยันการสร้างคำขอบันทึกเวลาหรือไม่ ?
          </Typography>
          {/* Action Buttons */}
...
export default TimeRecordConfirmModal

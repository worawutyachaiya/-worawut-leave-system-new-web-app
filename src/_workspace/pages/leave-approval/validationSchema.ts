...
const EmployeeLeaveSearchSchema = z.object({
...
export const validationSchemaPage = z.object({
  searchFilters: EmployeeLeaveSearchSchema,
    ...
    try {
      ...
      const columnFilters =
        result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults?.columnFilters?.map(
          (item: any) => {
            if (['CREATE_DATE', 'UPDATE_DATE', 'LEAVE_REQUEST_DATE'].includes(item.id)) {
      ...
      resolve({
        searchFilters: {
          ...
          startDate: savedData?.searchFilters?.startDate || dayjs().format('YYYY-MM-DD'),

  ...
  const handleExport = () => {
    ...
    const exportData = allData.map(row => {
      ...
      if (!dateRange && row.LEAVE_REQUEST_START_DATE) {
        ...
        const endDate = row.LEAVE_REQUEST_END_DATE ? dayjs(row.LEAVE_REQUEST_END_DATE).format('DD/MM/YYYY') : startDate
    ...
    >
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4, mt: 2 }}>
          ...
          {isError && <Box sx={{ color: 'error.main' }}>{t('Error loading data for export')}</Box>}
        </Box>
      </DialogContent>
      <DialogActions>

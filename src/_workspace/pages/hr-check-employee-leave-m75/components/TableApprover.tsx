    ...
    resultArray.forEach(el => {
      const apiItem = apiData.find((item: any) => {
        const apiApproverId = isFlexTime
          ? item.FLEX_TIME_APPROVAL_BY || item.APPROVAL_BY_APPROVER_EMPLOYEE_CODE
          : item.APPROVAL_BY_APPROVER_EMPLOYEE_CODE
        return el.APPROVER_ID.toString().toLowerCase() === apiApproverId?.toLowerCase()
      })

...
import { SubordinateFlexTime } from '@/_workspace/types/subordinate-flex-time/SubordinateFlexTimeTypes'
interface TableApproverProps {
  row: FlexTimeRequestData | SubordinateFlexTime
}
...
const TableApprover: React.FC<TableApproverProps> = ({ row }) => {
  ...
  const params = useMemo(
    ...
    [row?.FLEX_TIME_REQUEST_ID]
  )
  const approverList = row?.APPROVER || []
  const shouldFetch = !!row?.FLEX_TIME_REQUEST_ID && approverList.length >= 0
  const { data, isLoading, isError } = useSearchFlexTimeApprover(params, shouldFetch)
  const result = useMemo<ApproverItem[]>(() => {
    const resultArray: ApproverItem[] = []
    const apiData = (data as any)?.data?.ResultOnDb || []
    if (row?.IS_APPROVED === 0 || row?.IS_APPROVED === ('0' as any)) {
